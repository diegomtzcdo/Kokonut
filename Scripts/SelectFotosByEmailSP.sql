...
BEGIN
	...
	SELECT F.id_foto, F.name, F.latitude, F.longitude, F.enabled FROM kokonutPrueba.dbo.fotos F INNER JOIN kokonutPrueba.dbo.usuarios_fotos UF 
		ON F.id_foto = UF.usuario_id_usuario INNER JOIN kokonutPrueba.dbo.usuarios U ON UF.usuario_id_usuario = U.id_usuario WHERE
		U.email = @email ORDER BY F.id_foto OFFSET @offset ROWS FETCH NEXT @size ROWS ONLY;
