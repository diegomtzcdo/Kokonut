...
public class ErrorControlador /*extends ResponseEntityExceptionHandler*/ {
    ...
    @ExceptionHandler({RecursoNoLocalizadoExcepcion.class})
    public ResponseEntity<DetallesErrorBean> handle(RecursoNoLocalizadoExcepcion e, WebRequest request) {
        return new ResponseEntity<>(DetallesErrorBean.builder().timestamp(new Date()).message(e.getMessage()).
                details(request.getDescription(true)).build(), HttpStatus.NOT_FOUND);
    }

    @ExceptionHandler({ClaveDuplicadaExcepcion.class})
    public ResponseEntity<DetallesErrorBean> handle(ClaveDuplicadaExcepcion e, WebRequest request) {
        return new ResponseEntity<>(DetallesErrorBean.builder().timestamp(new Date()).message(e.getMessage()).
